...
// Tagged value type for the VM — supports integers, floats, strings, and heap pointers
...
struct Value {
    ...
    explicit Value(const std::string& val) : type(ValueType::String), i(0), str(val) {}
    ...
    int64_t asInt() const {
        switch (type) {
            ...
            case ValueType::Float:   return static_cast<int64_t>(f);
            case ValueType::HeapPtr: return static_cast<int64_t>(ptr);
            default: return 0;
        }
    }
    ...
    double asFloat() const {
        switch (type) {
            ...
            case ValueType::Integer: return static_cast<double>(i);
            default: return 0.0;
        }
    }
    ...
    std::string asString() const {
        switch (type) {
            ...
            case ValueType::Integer: return std::to_string(i);
            case ValueType::Float:   return std::to_string(f);
            case ValueType::HeapPtr: return "ptr@" + std::to_string(ptr);
            case ValueType::ArrayRef:return "array#" + std::to_string(ptr);
...
struct ConstantPool {
    ...
    const Value& get(uint16_t index) const {
        if (index >= constants.size())
            throw std::out_of_range("Constant pool index out of range: " + std::to_string(index));
