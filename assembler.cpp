...
Assembler::AssembleResult Assembler::assemble(const std::string& source) {
    ...
    auto skipCommasAndNewlines = [&]() {
        while (ti < tokens.size() &&
               (tokens[ti].type == Token::COMMA || tokens[ti].type == Token::NEWLINE))
            ++ti;
    };
    auto expectComma = [&]() {
    ...
    while (ti < tokens.size()) {
        ...
        switch (instr.opcode) {
            // No operands
            case OpCode::NOP: case OpCode::HALT: case OpCode::RET:
            case OpCode::DUP: case OpCode::SWAP: case OpCode::ROT:
            case OpCode::DEPTH: case OpCode::LEAVE:
            case OpCode::DUMP_REGS: case OpCode::DUMP_STACK: case OpCode::DUMP_HEAP:
            case OpCode::TRACE_ON: case OpCode::TRACE_OFF: case OpCode::ENDTRY:
            case OpCode::GC_COLLECT: case OpCode::GC_ON: case OpCode::GC_OFF:
            case OpCode::GC_STATS:
                break;

            // Single register operand
            case OpCode::PUSH: case OpCode::POP: case OpCode::INC: case OpCode::DEC:
            case OpCode::NEG: case OpCode::FNEG: case OpCode::FREE:
            case OpCode::PRINT: case OpCode::PRINTLN:
            case OpCode::INPUT: case OpCode::INPUTF: case OpCode::INPUTS:
            case OpCode::THROW: case OpCode::DEBUG: {
            ...
            case OpCode::JG: case OpCode::JL: case OpCode::JGE: case OpCode::JLE:
            case OpCode::JO: case OpCode::JNO:
            case OpCode::CALL: case OpCode::TRY: {
                auto op = parseOperand(tok.line);
                instr.op1 = op.value;
                break;
            }
            ...
            case OpCode::XCHG:
            case OpCode::ABS: case OpCode::FABS: case OpCode::FSQRT:
            case OpCode::FSIN: case OpCode::FCOS:
            case OpCode::ITOF: case OpCode::FTOI: case OpCode::ITOS: case OpCode::FTOS:
            case OpCode::NOT:
            case OpCode::CMP: case OpCode::FCMP: case OpCode::TEST:
            case OpCode::ALLOC: case OpCode::ANEW: case OpCode::ALEN: {
            ...
            case OpCode::MOVI: case OpCode::LOAD_CONST: {
                auto op1 = parseOperand(tok.line);
                instr.op1 = op1.value;
                expectComma();
                auto op2 = parseOperand(tok.line);
            ...
            case OpCode::FDIV: case OpCode::FMOD: case OpCode::FPOW:
            case OpCode::AND: case OpCode::OR: case OpCode::XOR:
            case OpCode::SHL: case OpCode::SHR: case OpCode::SAR:
            case OpCode::ROL: case OpCode::ROR:
            case OpCode::MEMCPY: case OpCode::MEMSET:
            case OpCode::REALLOC:
            case OpCode::AGET: case OpCode::ASET:
            case OpCode::LEA: {
                auto op1 = parseOperand(tok.line);
                instr.op1 = op1.value;
                expectComma();
            ...
            case OpCode::LOADH: {
                ...
                instr.op3 = op3.isImmediate ? static_cast<uint16_t>(op3.rawInt) : op3.value;
                break;
            }
            ...
            case OpCode::STOREH: {
                ...
                auto op2 = parseOperand(tok.line);
                // Use raw integer for the offset, not constant pool index
                instr.op2 = op2.isImmediate ? static_cast<uint16_t>(op2.rawInt) : op2.value;
                expectComma();
                auto op3 = parseOperand(tok.line);
                instr.op3 = op3.value;
                break;
            ...
            default:
                throw AssemblerError(tok.line, "Unhandled opcode in assembler: " + tok.text);
        }
                                    ...
                                    const ConstantPool& constants)
{
    ...
    for (size_t i = 0; i < program.size(); ++i) {
        ...
        switch (instr.opcode) {
            ...
            case OpCode::PUSH: case OpCode::POP: case OpCode::INC:
            case OpCode::DEC: case OpCode::NEG: case OpCode::FNEG:
            case OpCode::FREE: case OpCode::PRINT: case OpCode::PRINTLN:
            case OpCode::INPUT: case OpCode::INPUTF: case OpCode::INPUTS:
            case OpCode::THROW: case OpCode::DEBUG:
            ...
            case OpCode::JMP: case OpCode::JZ: case OpCode::JNZ:
            case OpCode::JG: case OpCode::JL: case OpCode::JGE: case OpCode::JLE:
            case OpCode::JO: case OpCode::JNO: case OpCode::CALL: case OpCode::TRY:
                out << "@" << instr.op1;
                break;
            ...
            case OpCode::PICK: case OpCode::ENTER: case OpCode::SYSCALL:
                out << "#" << instr.op1;
                break;

            case OpCode::MOV: case OpCode::LOAD: case OpCode::STORE:
            case OpCode::XCHG: case OpCode::ABS: case OpCode::FABS:
            case OpCode::FSQRT: case OpCode::FSIN: case OpCode::FCOS:
            case OpCode::ITOF: case OpCode::FTOI: case OpCode::ITOS: case OpCode::FTOS:
            case OpCode::NOT: case OpCode::CMP: case OpCode::FCMP: case OpCode::TEST:
            case OpCode::ALLOC: case OpCode::ANEW: case OpCode::ALEN:
