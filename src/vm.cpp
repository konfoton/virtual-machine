...
void VirtualMachine::push(const Value& val) {
  if (sp_ >= stack_.size()) throw std::overflow_error("Stack overflow");
  stack_[sp_++] = val;
...
Value VirtualMachine::pop() {
  if (sp_ == 0) throw std::underflow_error("Stack underflow");
...
const Value& VirtualMachine::peek(size_t offset) const {
  if (offset >= sp_) throw std::out_of_range("Stack peek out of range");
...
Value& VirtualMachine::heapAt(uint64_t addr) {
  if (addr >= heap_.size())
    throw std::out_of_range("Heap access out of range: " +
                            std::to_string(addr));
  return heap_[addr];
}
    ...
    for (size_t idx = 0; idx < heapBlocks_.size(); ++idx) {
      if (!heapBlocks_[idx].used && heapBlocks_[idx].size >= size) {
        ...
        if (heapBlocks_[idx].size > size + 4) {
          HeapBlock newBlock{addr + size, heapBlocks_[idx].size - size, false,
                             false};
          heapBlocks_[idx].size = size;
          heapBlocks_[idx].used = true;
          heapBlocks_.insert(heapBlocks_.begin() + static_cast<long>(idx + 1),
                             newBlock);
        } else {
          heapBlocks_[idx].used = true;
        }
...
void VirtualMachine::heapFree(uint64_t addr) {
  ...
  throw std::runtime_error("Heap free: invalid address " +
                           std::to_string(addr));
}
...
uint64_t VirtualMachine::heapRealloc(uint64_t addr, uint64_t newSize) {
  ...
  if (oldSize == 0) throw std::runtime_error("Heap realloc: invalid address");

  uint64_t newAddr = heapAlloc(newSize);
  uint64_t copySize = std::min(oldSize, newSize);
  for (uint64_t i = 0; i < copySize; ++i) heap_[newAddr + i] = heap_[addr + i];
  heapFree(addr);
  return newAddr;
}
...
VirtualMachine::ExecResult VirtualMachine::run() {
  ...
  while (running_) {
    auto result = step();
    if (result != ExecResult::OK) return result;
  }
...
VirtualMachine::ExecResult VirtualMachine::dispatch(const Instruction& instr) {
  try {
    switch (instr.opcode) {
      ...
      case OpCode::SYSCALL: {
        ...
        if (it == syscalls_.end())
          throw std::runtime_error("Unknown syscall: " +
                                   std::to_string(instr.op1));
        it->second(*this);
        break;
      }

      case OpCode::DEBUG:
        std::cerr << "[DEBUG] R" << instr.op1 << " = " << registers_[instr.op1]
                  << std::endl;
        break;
      ...
      case OpCode::LEA: {
        int64_t addr =
            registers_[instr.op2].asInt() + static_cast<int16_t>(instr.op3);
      ...
      case OpCode::FADD: {
        double result =
            registers_[instr.op2].asFloat() + registers_[instr.op3].asFloat();
        registers_[instr.op1] = Value(result);
      ...
      case OpCode::AND: {
        int64_t result =
            registers_[instr.op2].asInt() & registers_[instr.op3].asInt();
        registers_[instr.op1] = Value(result);
        setFlags(result);
        break;
      }
      ...
      case OpCode::ROL: {
        uint64_t val = static_cast<uint64_t>(registers_[instr.op2].asInt());
        uint64_t shift =
            static_cast<uint64_t>(registers_[instr.op3].asInt()) % 64;
        int64_t result =
            static_cast<int64_t>((val << shift) | (val >> (64 - shift)));
        registers_[instr.op1] = Value(result);
        setFlags(result);
        break;
      }
      ...
      case OpCode::TEST: {
        int64_t result =
            registers_[instr.op1].asInt() & registers_[instr.op2].asInt();
        setFlags(result);
        break;
      }
      ...
      case OpCode::CALL: {
        ...
        frame.savedFramePointer =
            static_cast<uint64_t>(registers_[RFP].asInt());
      ...
      case OpCode::MEMCPY: {
        ...
        for (uint64_t i = 0; i < size; ++i) heap_[dst + i] = heap_[src + i];
        break;
      }
      ...
      case OpCode::MEMSET: {
        ...
        uint64_t size = static_cast<uint64_t>(registers_[instr.op3].asInt());
        for (uint64_t i = 0; i < size; ++i) heap_[addr + i] = val;
        break;
      }
      ...
      case OpCode::ENDTRY:
        if (!exceptionHandlers_.empty()) exceptionHandlers_.pop_back();
        break;
      ...
      case OpCode::ALEN: {
        uint64_t id = registers_[instr.op2].ptr;
        auto& arr = getArray(id);
        registers_[instr.op1] =
            Value(static_cast<int64_t>(arr.elements.size()));
        break;
      }
      ...
      case OpCode::DUMP_STACK: {
        ...
        for (size_t i = sp_; i > 0; --i) {
          std::cerr << "  [" << std::setw(4) << (i - 1) << "] " << stack_[i - 1]
                    << std::endl;
        }
        break;
      }
      ...
      case OpCode::DUMP_HEAP: {
        ...
        for (const auto& block : heapBlocks_) {
          std::cerr << "  addr=" << block.address << " size=" << block.size
      ...
      case OpCode::GC_STATS:
        std::cout << "[GC Stats] collections=" << stats_.gcCollections
                  ...
                  << " arrays=" << arrays_.size() << std::endl;
        break;

      default:
        throw std::runtime_error("Unknown opcode: 0x" + [](uint8_t v) {
          char buf[8];
    ...
    std::cerr << "[VM ERROR] PC=" << (pc_ - 1)
              << " Opcode=" << opcodeName(instr.opcode) << " : " << e.what()
              << std::endl;
    running_ = false;
...
void VirtualMachine::gcMarkValue(
    const Value& val, std::vector<uint64_t>& heapWorklist,
...
void VirtualMachine::gcMarkHeapBlock(
    uint64_t addr, std::vector<uint64_t>& heapWorklist,
    std::unordered_map<uint64_t, bool>& markedArrays) {
  for (auto& block : heapBlocks_) {
